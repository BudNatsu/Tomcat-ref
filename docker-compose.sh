...
read -e -p "Où souhaitez-vous créer ce répertoire ?" -i "/root/docker-tomcat/" rep
...
#read -e -p "Chemin du docker compose à utiliser :" -i "/root/docker-tomcat/" docker
#cp $docker/* $dc
...
read -e -p "Adresse d'elasticsearch :" -i "fagonie-bcs.v212.abes.fr" elasticsearch
echo "ELASTICSEARCH_HOSTS=$elasticsearch:9200" >> $dc/Tomcat-ref/.env

read -e -p "Adresse de kibana :" -i "fagonie-bcs.v212.abes.fr" kibana 
echo "KIBANA_HOST=$kibana:5601" >> $dc/Tomcat-ref/.env

read -e -p "Adresse de logstash :" -i "fagonie-bcs.v212.abes.fr" kibana 
